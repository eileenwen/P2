...
public class WordGramBenchmark {
	public final static int WSIZE = 10; //number of word grams
	...
	public static int benchmark(Set<WordGram> set, String filename) throws FileNotFoundException {
		...
		while (scan.hasNext()) {
			String s = scan.next();
			list.add(s);      //creates new arraylist of Strings that are entered via scanner
		}
		scan.close();
		String[] words = list.toArray(new String[0]); //converts list to an Array. num wordgrams you can actually make
		for(int k=0; k < words.length - WSIZE + 1; k += 1) { //why is the max words.length-WSIZE +1? prevents from going out of b
			WordGram wg = new WordGram(words,k,WSIZE);		//creating a new wordGram
			set.add(wg);									//creates set of wordgrams;
		}
		return words.length - WSIZE + 1; 	//returns how many wordgrams are made
	}
	public static int benchmarkShift(Set<WordGram> set, String filename) throws FileNotFoundException {
		Scanner scan = new Scanner(new File(filename));

		String[] words = new String[WSIZE];  //creating an array of String of size WSIZE from scanner
		for(int k=0; k < WSIZE; k += 1) {
			words[k] = scan.next(); //takes first words in the file and puts them in 1-3 in array words
		}

		int count = WSIZE;
		WordGram current = new WordGram(words,0,WSIZE); //creating the first wordgram
		set.add(current);
		
		while (scan.hasNext()) {
			String s = scan.next();		//asking to add strings
			current = current.shiftAdd(s);    //adds s to the end of the string
			set.add(current);                 //adds the wordgram to the set
										// += 1;						//size of set
		}
		scan.close();
